...
// GET method to retrieve the details of a fundraiser by ID including donations
router.get("/fundraiser/:id", (req, res) => {
  const fundraiserId = req.params.id;
  const sql = `
      SELECT F.*, D.donation_id, D.amount, D.donation_date 
      FROM fundraiser F
      LEFT JOIN donation D ON F.fundraiser_id = D.fundraiser_id
      WHERE F.fundraiser_id = ?
  `;
  db.query(sql, [fundraiserId], (err, results) => {
      if (err) {
...
router.delete('/fundraisers/:id', (req, res) => {
  ...
  const deleteDonationsQuery = 'DELETE FROM donation WHERE fundraiser_id = ?';
  db.query(deleteDonationsQuery, [id], (err, result) => {
    if (err) {
      console.error('Error deleting related donations:', err);
